...
DEFAULT_DIST_WEIGHT = 10  # Back to original value
...
# Reduced intensity for smoother free flight
@cuda.jit('float32(float32, float32)', device=True, inline=True)
def smooth_stage_cost(dist2, dist_weight):
  return 0.3 * dist_weight * dist2  # 70% reduction for smoother behavior

# Safety zone cost removed - no longer needed
...
def dynamics_update(x, u, dt, contact_normal, inertia_mass, plane, cf_out, pen_out):
  """Stochastic hybrid contact dynamics for hexarotor - Poisson jump process + stochastic forces"""
  ...
  # ───── Anti-Woodpecker Contact Model ───────────────────────────────
  # Hysteresis-based contact with NO oscillation
  
  # Contact detection with velocity-dependent activation
  vel_towards_surface = nx * x[3] + ny * x[4] + nz * x[5]
  
  # HYSTERESIS: Different thresholds for entering vs leaving contact
  # Enter contact: only when very close (5mm) and slow
  # Leave contact: only when far away (15mm) or moving away fast
  
  # Check if we should ENTER contact state
  if pen < 0.005:  # 5mm activation zone (much smaller!)
    if vel_towards_surface < -0.001:  # Very slow approach (1mm/s)
      # Enter contact state - apply minimal forces
      contact_factor = max(0.0, min(1.0, (0.005 - pen) / 0.005))  # 0 to 1
      
      # Extremely conservative force scaling
      force_scale = contact_factor * 0.02  # Much reduced (0.02 vs 0.1)
      
      # Minimal force rate control
      x[12] += dt * u[6] * force_scale  # ḟ_x
      x[13] += dt * u[7] * force_scale  # ḟ_y  
      x[14] += dt * u[8] * force_scale  # ḟ_z
      
      # Only apply spring forces when actually penetrating
      if pen < -0.002:  # Only when 2mm penetration
        # Extremely gentle spring-damper forces
        k_contact = 2.0   # Much lower stiffness (was 10)
        c_contact = 0.2   # Much lower damping (was 1)
        
        # NaN-safe force calculation with tight limits
        pen_safe = max(-0.01, min(0.0, pen))  # Clamp to 1cm max
        vel_safe = max(-0.5, min(0.5, vel_towards_surface))  # Clamp to 0.5m/s
        
        # Simple spring-damper forces
        spring_force = -k_contact * pen_safe
        damper_force = -c_contact * vel_safe
        
        # Apply forces with tight safety checks
        if abs(spring_force) < 10.0 and abs(damper_force) < 10.0:  # Tight limits
          x[12] += dt * (spring_force * nx + damper_force * nx) * 0.1
          x[13] += dt * (spring_force * ny + damper_force * ny) * 0.1
          x[14] += dt * (spring_force * nz + damper_force * nz) * 0.1
      
      # Very tight force limits
      max_force = 1.0  # Much lower limit (1N max)
      x[12] = max(-max_force, min(max_force, x[12]))
      x[13] = max(-max_force, min(max_force, x[13]))
      x[14] = max(-max_force, min(max_force, x[14]))
      
      # NaN-safe force check
      if x[12] != x[12] or x[13] != x[13] or x[14] != x[14]:  # NaN check
        x[12] = x[13] = x[14] = 0.0  # Reset to zero if NaN
      ...
      # NaN-safe moment calculation
      if abs(rx) < 3.0 and abs(ry) < 3.0 and abs(rz) < 3.0:  # Tight arm length check
        m_x = (ry * x[14] - rz * x[13]) * 0.01  # Much reduced moment scaling
        m_y = (rz * x[12] - rx * x[14]) * 0.01  # Much reduced moment scaling
        m_z = (rx * x[13] - ry * x[12]) * 0.01  # Much reduced moment scaling
        
        # NaN check for moments
        if m_x != m_x or m_y != m_y or m_z != m_z:  # NaN check
          m_x = m_y = m_z = 0.0
      else:
        m_x = m_y = m_z = 0.0
    else:
      # Moving away from surface - no contact forces
      m_x = m_y = m_z = 0.0
  # HYSTERESIS: Only leave contact when far away or moving fast
  elif pen > 0.015:  # 15mm threshold to leave contact (3x the entry threshold)
    # Force exit from contact state
    m_x = m_y = m_z = 0.0
  else:
    # In hysteresis zone - maintain current contact state
    # Gentle force decay
    x[12] *= 0.99  # Very gentle decay
    x[13] *= 0.99
    x[14] *= 0.99
    m_x = m_y = m_z = 0.0
    
  # Final NaN safety check for all forces and moments
  if x[12] != x[12]: x[12] = 0.0
  if x[13] != x[13]: x[13] = 0.0  
  if x[14] != x[14]: x[14] = 0.0
  if m_x != m_x: m_x = 0.0
  if m_y != m_y: m_y = 0.0
  if m_z != m_z: m_z = 0.0
...
class MPPI_Numba(object):
  ...
  def rollout_numba(
    ...
    for t in range(timesteps):
      ...
      # Penetration cost removed - let natural contact dynamics handle approach without attraction/repulsion
