...
def dynamics_update(x, u, dt, contact_normal, inertia_mass, plane, cf_out, pen_out):
  """Fast hybrid contact dynamics for hexarotor - minimal changes for speed"""
  ...
  # Hybrid contact: smooth activation + force rate control
  if pen < 0.2:
    # Smooth contact activation (avoid binary transitions)
    contact_strength = 1.0 - (pen + 0.2) / 0.4  # Smooth from 0 to 1
    contact_strength = max(0.0, min(1.0, contact_strength))
    
    # Force rate control (u[6:9] are force rates)
    x[12] += dt * u[6] * contact_strength   # ḟ_x
    x[13] += dt * u[7] * contact_strength   # ḟ_y
    x[14] += dt * u[8] * contact_strength   # ḟ_z

    # Contact moments (cross product of arm × force)
    rx = ee_x - x[0]
    ry = ee_y - x[1]
    rz = ee_z - x[2]
    m_x = ry * x[14] - rz * x[13]
    ...
    # No contact - decay forces smoothly
    x[12] *= 0.95  # Smooth decay
    x[13] *= 0.95
    x[14] *= 0.95
    m_x = m_y = m_z = 0.0
