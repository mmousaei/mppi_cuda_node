...
class ControlHexarotor:
    def __init__(self):
        ...
        self.cfg = Config(
            ...
            dt=0.04,  # Time step
            num_control_rollouts=1024*4,  # Number of control sequences to sample
       ...
       0.00479975]),
            ...
            'weights' : np.array([2.54400000e+04, 3.55382091e+04, 1.95739902e+04, 1.80000000e+03,
       ...
       5.66759663e+04]),                 # w_terminal
        ...
        # Second MPPI Controller for refinement (2-step MPPI)
        self.refinement_cfg = Config(
            T=0.1,  # Shorter horizon length for refinement
            dt=0.05,  # 2-step MPPI refinement
            num_control_rollouts=512,  # Fewer rollouts for faster optimization
            num_controls=6,
            num_states=12,
            num_vis_state_rollouts=1,
            seed=2
        )
        self.refinement_controller = MPPI_Numba(self.refinement_cfg)
        self.refinement_params = {
            'dt': self.refinement_cfg.dt,
            'x0': self.current_state,
            'xgoal': self.mppi_params['xgoal'],
            'goal_tolerance': 0.001,
            'dist_weight': 1000,  # Adjust weights for fine-tuning
            'lambda_weight': 10.0,
            'num_opt': 2,
            'u_std': self.mppi_params['u_std'] * 0.05,  # Smaller perturbations
            'vrange': self.mppi_params['vrange'],
            'wrange': self.mppi_params['wrange'],
            'weights': self.mppi_params['weights'], 
            "inertia_mass" : self.mppi_params['inertia_mass']
        }
        self.refinement_controller.set_params(self.refinement_params)
    ...
    def normalize_control_inputs(self, control_inputs, gravity_vector_body):
        ...
        control_inputs[3:6] = control_inputs[3:6] * 3
    ...
    def compute_control_hirarchial_mppi(self):
        # print("compute control")
        # print(f'current_state: {self.current_state[0]}, {self.current_state[1]}, {self.current_state[2]}')
        self.mppi_controller.shift_and_update(self.current_state, self.optimal_control_seq, num_shifts=1)
        self.optimal_control_seq = self.mppi_controller.solve()
        ...
        # Refinement with second MPPI (interpolation of initial sequence)
        self.refinement_controller.params['x0'] = self.current_state
        self.refinement_controller.params['xgoal'] = self.mppi_controller.params['xgoal']
        refined_seq_length = int(self.refinement_cfg.T / self.refinement_cfg.dt)
        interpolated_seq = np.zeros((refined_seq_length, self.cfg.num_controls))
        for i in range(refined_seq_length):
            interp_time = i * self.refinement_cfg.dt
            interp_idx = interp_time / self.cfg.T * (self.optimal_control_seq.shape[0] - 1)
            lower_idx = int(np.floor(interp_idx))
            upper_idx = int(np.ceil(interp_idx))
            alpha = interp_idx - lower_idx
            interpolated_seq[i, :] = (1 - alpha) * self.optimal_control_seq[lower_idx, :] + alpha * self.optimal_control_seq[upper_idx, :]
        self.refinement_controller.shift_and_update(self.current_state, interpolated_seq, num_shifts=1)
        refined_control_seq = self.refinement_controller.solve()
        control_inputs = refined_control_seq[0, :]  # Refined control inputs
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.compute_control_hirarchial_mppi()
