...
class MPPIControllerNode(object):
    def __init__(self):
        ...
        self.mppi_params = {
            ...
            'plane': np.array([-1, 0, 0, 3.05]),
            'goal_tolerance': 0.001,
            'dist_weight': 100,  # Reduced from 2000 to 100 (95% reduction)
            'lambda_weight': 5,   # Reduced from 50 to 5 (90% reduction)
            'num_opt': 5,
            'u_std': np.array([2, 2, 2, 0.05, 0.05, 0.05, 0.1, 0.1, 0.1]),
            'vrange': np.array([-10.0, 10.0]),
            'wrange': np.array([-0.1, 0.1]),
            'weights': np.array([
                100, 100, 100,      # Position weights: reduced from 19550/24840 to 100 (99%+ reduction)
                1, 1, 1,            # Velocity weights: kept low
                100, 100, 100,      # Attitude weights: reduced from 95500 to 100 (99%+ reduction)
                1, 1, 1,            # Angular velocity weights: kept low
                1, 100, 1, 100, 200, # Force/moment weights: kept as is
                100, 100, 100       # Terminal weights: reduced from 5000 to 100 (98% reduction)
            ]),
        ...
        self.target_force_pub = rospy.Publisher('/mpc/wrenchtarget', WrenchStamped, queue_size=1)
    ...
    def publish_mpc_target(self):
        ...
        # Contact detection based on current contact plane (X = 1.8m threshold)
        if curr[0] > 1.8:
            self.contacting = 1
        else:
            self.contacting = 0 
